...
**[Download the latest release](https://github.com/aleixrodriala/desktop/releases/latest)**
...
This fork doesn't diverge from upstream GitHub Desktop. It applies a set of patch commits on top of each upstream release:

1. CI checks for new upstream `release-*` tags every 6 hours
2. Cherry-picks the WSL patches onto the new tag
3. If clean → auto-builds and publishes a new release
4. If conflict → opens a GitHub Issue for manual resolution

The patches are designed to be minimal and conflict-resistant: one new file (`wsl.ts`), one new directory (`wsl-daemon/`), and small surgical changes to existing files (mostly import swaps).
